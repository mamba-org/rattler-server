...
/// order of `packages` and of the `depends` vector inside `RepoDataRecord`
pub fn sort_topologically(packages: Vec<RepoDataRecord>) -> Vec<RepoDataRecord> {
    let roots = get_graph_roots(&packages);

    let mut all_packages = packages
        .into_iter()
        .map(|p| (p.package_record.name.clone(), p))
        .collect();
...
fn get_graph_roots(records: &[RepoDataRecord]) -> Vec<String> {
    let all_packages: HashSet<_> = records
        .iter()
        .map(|r| r.package_record.name.as_str())
        .collect();

    let dependencies: HashSet<_> = records
        .iter()
        .flat_map(|r| {
            r.package_record
                .depends
...
fn get_topological_order(
    ...
    packages: &mut HashMap<String, RepoDataRecord>,
) -> Vec<RepoDataRecord> {
...
mod test {
    ...
    fn sanity_check_topological_sort(
        ...
        let all_sorted_packages: HashSet<_> = sorted_packages
            .iter()
            .map(|p| p.package_record.name.as_str())
            .collect();
        let all_original_packages: HashSet<_> = original_packages
            .iter()
            .map(|p| p.package_record.name.as_str())
            .collect();
    ...
    fn simulate_install(
        sorted_packages: &[RepoDataRecord],
        circular_dependencies: &HashSet<(&str, &str)>,
    ) {
        let packages_by_name: HashMap<_, _> = sorted_packages
            .iter()
            .map(|p| {
                (
